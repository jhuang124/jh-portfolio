...
vi.mock("../ToolCallBadge", () => ({
  ToolCallBadge: ({ toolName, args, state }: { toolName: string; args: Record<string, unknown>; state: string }) => {
    const path = (args.path as string) ?? "file";
    const isCreate = args.command === "create";
    const isDone = state === "result";
    let label: string;
    if (toolName === "str_replace_editor") {
      label = isDone ? (isCreate ? `Created ${path}` : `Updated ${path}`) : (isCreate ? `Creating ${path}…` : `Editing ${path}…`);
    } else {
      label = toolName;
    }
